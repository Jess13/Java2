...
public class Icons1 {
    ...
    public static void main(String[] args) {
        ...
        ImageIcon image = new ImageIcon("src\\Tests\\Icons1\\128.png"); //значок иконки в заголовке
        frame.setIconImage(image.getImage());
        //frame.setLocationRelativeTo(null);
//frame.setResizable(false); //позволяет изменять размер

        try { //создает фон программы
            frame.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("src\\Tests\\Icons1\\knopka.jpg")))));
        } catch (IOException e) {
            e.printStackTrace();
        }
